...
<div id="all_rentals_page">
    <div class="container">
        <div class="row">
            ...
            <?php
            $sql = $pdo->prepare("SELECT * FROM all_properties ORDER BY date_added DESC");
            $sql->execute();
            $database_properties = $sql->fetchAll(PDO::FETCH_ASSOC);
            ?>
            <?php foreach ($database_properties as $all) : ?>
                <div class="col-md-3">
                    <div class="all_properties_details">
                        <a href="index.php?page=all_properties/individual_property&id=<?= $all['id']; ?>">
                            <img src="<?= $all['propertyImage1']; ?>" alt="<?= $all['propertyName']; ?>" class="img-fluid img-responsive img-top">
                            <h3><?= $all['propertyName']; ?></h3>
                            <h4><?= $all['propertyType']; ?></h4>
                        </a>
                    </div>
                </div>
            <?php endforeach; ?>
