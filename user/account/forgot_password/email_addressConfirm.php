...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($emailAddress_error)) {
        ...
        $sql = ("SELECT id from users WHERE emailAddress = :emailAddress");
        if ($stmt = $pdo->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                if ($stmt->rowCount() == 1) {
                    ...
                    if ($row = $stmt->fetch()) {
                        $id = $row['id'];
                        // Redirect the user to the change password page with their id in the url for unique identification
                        header("location: index.php?page=user/account/forgot_password/change_password&id=$id");
                        exit;
                    }
                } else {
                    $emailAddress_error = "Email Address does not exist!";
                }
            } else {
...
<!-- Header Template -->
<?= header_template('USER | EMAIL ADDRESS CONFIRMATION') ?>
...
<div class="back_link">
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                <a href="index.php?page=user/account/login"><i class="bi bi-arrow-left-circle"></i> Back</a>
            </div>
        </div>
    </div>
</div>
...
<div id="login">
    <div class="container">
        <div class="row">
            <div class="col-md-5">
                <div class="title">
                    <h5>User Email Confirmation</h5>
                    <hr>
                </div>

                <form action="index.php?page=user/account/forgot_password/email_addressConfirm" method="post" class="login-form">
                    <!-- Errors
                    ...
                    <div class="form-group my-3">
                        <label for="Emal Address">Email Address</label>
